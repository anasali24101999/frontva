...
(function ($) {
	...
	// Demo buy now button 
	// ====================
	if ($('#tt-demo-buy-btn').length) return;

	// Default values for CSS variables when they are not defined
	if (typeof getComputedStyle(document.documentElement).getPropertyValue('--tt-dark-color') === '') {
	  document.documentElement.style.setProperty('--tt-dark-color', '#222222');
	}
	if (typeof getComputedStyle(document.documentElement).getPropertyValue('--tt-light-color') === '') {
	  document.documentElement.style.setProperty('--tt-light-color', '#ffffff');
	}
	...
	const styles = `
	  ...
	  #tt-demo-buy-btn:hover .tt-demo-buy-btn-slider {
	...
	// HTML markup
	const markup = `
		<a href="https://1.envato.market/hektor" id="tt-demo-buy-btn" class="tt-hide-cursor" target="_blank" rel="nofollow noopener">
			<div class="tt-demo-buy-btn-trigger">
				${cartIconSVG}
			</div>
			<div class="tt-demo-buy-btn-slider">
				Buy Now!
			</div>
		</a>
	`;
	...
	// Add magic cursor hover behavior for the demo button
	$(document).on("mouseenter", "#tt-demo-buy-btn", function() {
		gsap.to("#ball", { duration: 0.3, scale: 0, opacity: 0 });
	}).on("mouseleave", "#tt-demo-buy-btn", function() {
